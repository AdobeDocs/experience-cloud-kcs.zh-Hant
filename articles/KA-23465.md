...
我們的一位AEMaaCS客戶會定期封鎖復寫佇列。 本文會說明工程團隊建議的因應措施，這些措施可使用管理員許可權在UI中執行。
...
我們的一位AEMaaCS客戶復寫佇列因為javax.jcr.NamespaceException：未知的名稱空間前置詞：ns22_例外而定期封鎖。
...
取消封鎖佇列只需要從golden-publish發佈佇列UI中刪除請求即可。 請建議客戶執行下列步驟。
