---
title: SMPP協定分析使用 [!DNL Wireshark]
description: 說明
solution: Campaign
product: Campaign
...
# SMPP協定分析使用 [!DNL Wireshark]

## 說明

<br>了解如何使用 [!DNL Wireshark].<br><br><br><br><br>簡介<br><br><br><br> <br><br>
大多數高吞吐量SMS-C與SMPP協定3.4版相容。此協定允許發送SMS並接收有關這些SMS的傳送的資訊。 SMPP協定記錄在SMPP協定規範v3.4中，該規範在Internet上作為PDF文檔提供。

本文不能替代本說明：給出了解釋協定規範並與協定規範匹配的實用提示 [!DNL Wireshark] 顯示以協助疑難排解Adobe Campaign與SMS-C合作夥伴之間的問題。
...
<br><br>捕獲網路流量，而 [!DNL Wireshark]<br><br><br><br><br><br>
如果您沒有直接存取電腦的權限，則可能需要使用命令列工具(例如 *tcpdump*. 如果已知連接的TCP埠，請放置正確的過濾器以避免捕獲所有通信。 以下是範例 `tcpdump` 捕獲埠的命令行 `12345` to <b>`outfile.pcap`</b>:
...
檔案 <b>`outfile.pcap`</b> 然後可在中開啟 [!DNL Wireshark] 供進一步分析。


<br><br>[!DNL Wireshark] 處理<br><br><br><br> <br><br><br><br>
本主題假設您熟悉 [!DNL Wireshark]:捕獲資料包，定義簡單過濾器，讀取資料包詳細資訊。 以下提供簡要介紹： [howtogeek — 如何使用 [!DNL Wireshark] 捕獲、過濾和Inspect資料包](https://www.howtogeek.com/104278/how-to-use-wireshark-to-capture-filter-and-inspect-packets/).

若要篩選掉 [!DNL Wireshark]，有三個重要功能：

- 在SMS-C的埠上使用顯示篩選器。例如，如果SMS-C使用埠 `10000`，請使用下列篩選器：
   *`tcp.port == 10000`*
...
該協定通過TCP運行，且是完全二進位的，這意味著特殊工具，如 [!DNL Wireshark] （或十六進位編輯器）需要解密資料流的內容。

資料流由獨立PDU組成：每個PDU都是一條消息，其中包含一個命令、一個狀態、一個序列號以及基於該命令的其他資訊。

由於TCP作為流協定的性質，TCP資料包可能包含多個PDU,PDU可能跨越2個或多個TCP資料包。 [!DNL Wireshark] 會正確地重新組裝PDU，因此對於 [!DNL Wireshark] 使用者。
...
`BIND_TRANSMITTER` 確認 *`BIND_TRANSMITTER_RESP`*, *`SUBMIT_SM`* 確認 *`SUBMIT_SM_RESP`*、等

回應有逾時，通常為10、30或60秒。回應可能包含正確確認(`command _status = 0`)或錯誤(請參閱5.1.3 *`command_status`*, *表5-2* （在SMPP規範中）。 大多數情況下，這些回應都會立即執行，且未達到回應逾時。
...
SMPP連接通過使用TCP連接開始。 然後 `BIND` 操作發送者 [!DNL Campaign]，確認 `BIND RESP`. 這些操作在SMPP規範第4.1節(`BIND` 操作)。
...
在 [!DNL Campaign]，您應會看到 *`BIND_TRANSMITTER`* 啟動MT傳輸和 *`BIND_RECEIVER`* 封包 *`nlsm s`* 觸發MO/SR連線。

<b>發送器、接收機和收發器： </b>用於Campaign Classic的SMPP連接器在單獨的發送器/接收器模式下工作：有兩個TCP連接，一個用於發送MT，另一個用於接收MO和SR。 請注意，TCP連接始終由 [!DNL Campaign]，即使是接收器模式亦然。
...
接收者被綁定時，SMS-C可隨時發送MO。 MO是使用 *`DELIVER_SM`* 位數為2-5的PDU *`esm_clas s`* 清晰(通常 *`esm_class`* 將僅為0)。
...
SMS訊息使用特殊的7位元編碼，通常稱為GSM7編碼。 請參閱 [!DNL Wikipedia] GSM 03.38（英文）。
<br><br><br><br> <br><br>
...
- 不支援GSM7編碼 [!DNL Wireshark]:特殊字元的顯示不正確。 如果您需要檢查GSM7字串是否正確編碼，則必須將十六進位代碼與GSM7表進行比較。


此 *`data_coding`* 欄位會告訴您使用的編碼。 唯一的問題是，值0表示規格中的預設SMS-C編碼，但一般來說是指GSM7。 向SMS-C *合作夥伴確認與哪些編碼相關聯 `data_coding = 0` (Adobe Campaign僅支援GSM7 `data_coding* = 0`)。
...
[!DNL Wikipedia] 提供有關使用者資料標題和串連SMS（英文）的好文章。

要了解short_message是否包含UDH，請檢查esm_class的位6和7（請參閱規範的5.2.12節）。 [!DNL Wireshark] 在介面中剖析UDH，給出了準確的資訊。
...
在上面的螢幕擷取中，您可以在訊息欄位(1)中看到使用者資料標題、UDH(2)中包含的資訊，以及不屬於資料包但由 [!DNL Wireshark] (3):「簡訊內文」欄位特別有趣，因為它包含重新組合的完整訊息 [!DNL Wireshark].
<br><br><br><br><br><br>接收SR<br><br><br><br><br><br>
當接收機綁定時，SMS-C可以隨時發送SR。 SR使用DELIVER_SM PDU發送，位數為2-5(*)`esm_class`*設定。
...
您應直接向SMS-C合作夥伴索取其實作的檔案，並仔細檢查其是否符合您在 [!DNL Wireshark]. SMS-C實作者甚至不知道其實作，這經常會導致問題和誤解。 如果對此欄位有任何疑問（尤其是錯誤碼），請務必向SMS-C合作夥伴尋求協助。
...
- id與*中已傳送的相同`SUBMIT_SM_RESP`*匹配的MT。
- 您可以忽略 `text` 欄位：此欄位被忽略 [!DNL Campaign] 因為它無用、不可靠，而且如果SMS是使用純英數字ASCII以外的其他編碼傳送，則甚至可能完全無法讀取。 這是正常行為。
- 欄位名稱不區分大小寫(例如 `id`: `sub`: `Text`:也可指出為 `ID`: `SUB`: `text`:)。
- *`dlvrd`*除非SMS-C合作夥伴記錄，否則欄位通常不可靠。
- 日期可能有任何時區，因此幾乎毫無用處，或者因為遠程伺服器的時鐘關閉而完全錯誤。
- *`stat`*欄位可能有附錄B中定義的其他值。使用常識和SMSC合作夥伴的文檔來了解其含義。
- *`err`*欄位完全取決於SMS-C，而SMS-C合作夥伴記錄的大部分時間。 代碼000通常表示成功，而其他任何代碼表示錯誤。 欄位通常為數值，但也可以是十六進位。
