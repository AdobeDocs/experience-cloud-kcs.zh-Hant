...
keywords: KCS、SMPP通訊協定、Wireshark、分析、ACC、Adobe Campaign Classic、做法
...
- 若要依電話號碼或文字內容隔離資料封包，請使用包含以下設定的搜尋功能：<br>\
...
- 使用 <b>追隨TCP資料流</b> 工具以隔離您正在處理的資料流。<br><br>    關閉彈出的紅色/藍色文字視窗，因為它只對與SMPP無關的文字通訊協定有用。
...
串流由獨立的PDU組成：每個PDU都是一則訊息，包含命令、狀態、序列號和基於命令的其他資訊。
...
<b>傳送器、接收器和收發器： </b>Campaign Classic的SMPP聯結器會在單獨的傳送器/接收器模式下運作：有兩個TCP連線，一個用於傳輸MT，另一個用於接收MO和SR。 請注意，TCP連線一律由Campaign起始，即使對於接收器模式亦然。
...
| 編碼 | data_coding | 訊息大小（字元） | 多部分SMS的部分大小 | 可用字元 |
| --- | --- | --- | --- | --- |
| GSM7 | 0 | 160 | 152 | [GSM7基本字元集+擴充功能](https://zh.wikipedia.org/wiki/GSM_03.38) （延伸字元佔2個字元） |
