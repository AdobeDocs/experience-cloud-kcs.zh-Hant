...
[&lt; — 上一個：什麼是「The Dispatcher」](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17911.html?lang=zh-Hant)

## 說明

本檔案將說明AMS標準配置檔案集以及此配置標準背後的思路
...
此目錄包含 `_cache.any`, `_invalidate.any` 您所建立的檔案，其中包含您要模組如何處理來自AEM的快取元素及失效規則語法的規則。  有關此部分的更多詳細資訊為 [此處](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#configuring-the-dispatcher-cache-cache)
...
即，在更新安裝期間，RPM將查看 `httpd.conf` 如果在 `unaltered` 會 *replace* 檔案上會有重要更新。  若 `httpd.conf` was `altered` 然後 *不會取代* 檔案，而是會建立名為 `httpd.conf.rpmnew` 且所需的修正將會位於服務啟動時不適用的檔案中。
...
[Next ->配置檔案的說明](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17477.html?lang=zh-Hant)