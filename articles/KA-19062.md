...
1. 使用rmNodes.groovy刪除最後一個命令輸出中列出的所有已標識的損壞節點
...
對於損壞的路徑，上述rmNode命令應返回true，這表示它已將其刪除。 通過在這些路徑上重新運行rmNode命令，確保刪除這些發現的三個損壞路徑。 下次執行它應該會傳回 false。

如果您發現儲存庫中仍有相同的路徑，請使用 <b>oak-run jar的修補版本，即oak-run-1.2.18-NPR-17596</b>. 此版本的jar會在壓縮作業中略過無法讀取的二進位檔，將其取代為0位元組二進位檔，並記錄例外狀況和syser的路徑。 產生的壓縮存放庫應接著傳遞oak-run檢查和節點計數指令碼，您也應能使用未修補的oak-run再次壓縮。
...
1. 透過使用以下查核點清單來執行查核點清理。如果有多個查核點，請加以清除：
   ...
   ```
2. 執行離線壓縮。 如果您不知道如何執行離線壓縮，請參閱 [此處](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. 啟動伺服器等待索引完成。
