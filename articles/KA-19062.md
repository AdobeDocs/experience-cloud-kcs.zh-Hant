...
錯誤記錄顯示SegmentNotFound例外狀況，如下列範例所示：
...
首先，使用Oak run工具，這是一個可執行的jar`[` 1`]`  其中包含您進行簡單Oak安裝及執行Oak相關作業所需的一切。

Oak-run的檢查執行模式可用於確定區段存放區的最後一個已知良好修訂。  這可用來手動將損壞的區段存放區恢復到其最新的良好修訂。

*注意：此程式會將系統中的資料復原到之前的時間點。  如果您想避免遺失系統中的變更，可以改為嘗試下面的選項B。*
...
2. 若要將損壞的區段存放區恢復到其最新的良好狀態，請變更為CQ的工作目錄（包含crx-quickstartfolder的工作目錄）並在備份中的所有檔案。/crx-quickstart/repository/segmentstore/。
3. 執行一致性檢查，
...
*在某些情況下，Oak執行檢查可能找不到良好的修訂，並且在執行檢查命令時，我們會收到「ConsistencyChecker — 找不到良好的修訂」。*

#### <b>如何修正一致性檢查中遇到「ConsistencyChecker — 找不到良好的修訂」時的損毀</b>

<b>手動移除損壞的節點。</b>

您可以在AEM、未設定FileDatastore的TarMK設定，以及二進位檔案中有損毀的情況中進行下列操作。

<b>注意</b>：以下程式適用於進階使用者。  刪除損壞的節點時，您需要確保它們不是系統節點（例如/home、/jcr：system等）。  如果它們是系統節點，您必須確定可以還原它們。  如果您不確定，請向AEM客戶服務團隊尋求此處記錄的步驟的協助。
...
2. 使用Oak run主控台並載入childCount groovy指令碼來識別區段存放區中的損壞節點：
   ...
   在某些情況下，該問題與二進位屬性有關，並且childCount groovy指令碼無法找到任何損壞的節點。  在這些情況下，您可以改用以下命令，它將讀取周遊期間遇到的每個二進位檔案的前1024個位元組（請注意，此命令會較慢，並且只應在上述命令未傳回預期結果時使用）：
...
3. 使用rmNodes.groovy移除最後一個命令輸出中列出的所有已識別損壞節點。使用以下命令載入oak-run主控台shell：
   ...
   注意：使用oak-run.jar 1.6.13版及更新版本時，請設定 — 讀寫JVM引數，如果您遇到錯誤，例如：
...
4. 對步驟2中找到的所有節點重複步驟3
   ...
   上面這個rmNode命令應該會針對損壞的路徑傳回true，這表示它刪除了它。 請務必透過在這些路徑上重新執行rmNode命令，刪除這些找到的三個損壞的路徑。 下次執行它應該會傳回 false。如果您發現存放庫中存在相同的路徑，請使用 <b>oak-run jar的修補版本，即oak-run-1.2.18-NPR-17596</b>. 此版本的jar在壓縮時會略過無法讀取的二進位檔案，將它們取代為0位元組的二進位檔案並記錄例外狀況和syserr的路徑。 產生的壓縮存放庫應會通過Oak-run檢查和節點計數指令碼，您也應能夠使用未修補的Oak-run再次壓縮它。
...
錯誤記錄檔中的SegmentNotFoundException表示區段已不存在，儘管顯然有人嘗試繼續存取。 這大致有三種不同的根本原因：區段已由手動介入移除（例如rm -rf /）、區段已由修訂垃圾收藏集移除，或由於程式碼中的一些錯誤而找不到區段。
