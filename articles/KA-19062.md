...
1. 通過編輯將儲存庫還原為此修訂 `./crx-quickstart/repository/segmentstore/journal.log` 和刪除包含最新良好修訂的行之後的所有行。

1. 全部移除 `./crx-quickstart/repository/segmentstore/\*.bak` 檔案。
...
\*注意n:\*以下過程適用於高級用戶。 刪除損壞的節點時，您需要確保這些節點不是系統節點(例如 `/home`, `/jcr:system`等)。 或者，如果它們是系統節點，則需要確保可以恢復它們。  如果您不確定，請洽詢AEM客戶服務團隊，以取得此處說明的步驟協助。
...
1. 使用刪除最後一個命令輸出中列出的所有已識別的損壞節點 `rmNodes.groovy`
   ...
   執行 `rmNode` 要刪除損壞的節點，請替換 `/path/to/corrupt/node` 使用需要刪除的損壞節點的路徑。
...
1. 對於在步驟2中找到的所有節點，重複步驟3。

   以上 `rmNode` 命令應為損壞的路徑返回true，這表示它已將其刪除。 通過在這些路徑上重新運行rmNode命令，確保刪除這些發現的三個損壞路徑。 下次執行時應會傳回false。

   如果您仍然看到儲存庫中存在相同的路徑，則使用打補丁的 [!DNL oak-run] jar。 `oak-run-1.2.18-NPR-17596`

   修補的版本為 [!DNL Oak] 運行Jar Do?

   此版本的jar會在壓縮時略過無法讀取的二進位檔，取代為0位元組二進位檔，並記錄例外狀況和syser的路徑。 因此，壓縮的存放庫應該會通過 [!DNL oak-run] 檢查，節點計數指令碼，您也應該能夠使用未修補程式再次壓縮它 [!DNL oak-run].

1. 使用下方列出查核點，以執行查核點清除。 如果有多個查核點，請加以清除：
