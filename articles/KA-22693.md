---
title: 「無法選取Adobe Target作為IMS設定的雲端解決方案」
description: 說明
...
# 無法選取Adobe Target作為IMS設定的雲端解決方案
...
客戶正嘗試建立Adobe Target IMS設定，以便將AEM與Adobe Target整合。

但是，每當他們選擇「Adobe Target」作為IMS設定的雲端解決方案時，就會彈出錯誤「無法擷取憑證：提供的userId未識別現有使用者」，並且無法建立設定。

只有Adobe Target解決方案會發生此問題。
...
您在ui中收到「無法擷取憑證」錯誤。 提供的使用者ID未識別現有使用者。」
...
檢查使用者「target-imsconfig-service」和「targetservice」的許可權時，注意到target-imsconfig-service未指派ACE。
...
路徑： /home/users/system/target-imsconfig-service
