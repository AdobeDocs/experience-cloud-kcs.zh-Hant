...
本文著重於AEM Dispatcher中最新的最佳化，以及如何最佳運用這些最佳化。  AEM Dispatcher是快取 [反向代理](https://stackoverflow.com/questions/224664/difference-between-proxy-server-and-reverse-proxy-server) 專為搭配Adobe Experience Manager使用而設計的伺服器。  它可以作為模組在現有Web伺服器軟體中安裝和運行。  在撰寫本文時， [dispatcher模組受支援](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-install.html) on [!DNL Apache HTTP Server]、 Microsoft IIS和 [!DNL iPlanet].
...
2. <b>針對不同時段快取個人化內容</b>  — 若您的網站有個人化內容，請考慮使用 [[!DNL Apache Sling Dynamic Includes]](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) 在AEM應用程式中 [!DNL Ajax] （在瀏覽器層級非同步JavaScript和XML呼叫）、SSI（在Web伺服器層級包含伺服器端）和ESI（在CDN層級包含邊緣端），以便在不同時間段內快取頁面的不同部分。
3. <b>絕不刪除即時Dispatcher上的Dispatcher快取</b>  — 如果Dispatcher提供即時內容，而您刪除快取，將會導致大量請求傳回AEM。  因此，絕不應在即時Dispatcher上刪除Dispatcher快取。
4. <b>提升快取 </b> — 刪除 [!DNL Dispatcher] 快取，提取 [!DNL Dispatcher] 在負載平衡器上，刪除快取，然後運行Web Crawler工具，在將dispatcher上的檔案放入負載平衡器之前快取它。
5. <b>快取錯誤頁</b>  — 善用 <b>[DispatcherPassError 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-install.html#ApacheWebServer) </b>([!DNL Apache] 特定於Web伺服器)的指令，用於從Dispatcher快取中提供錯誤頁面，例如404。
6. <b>GZip壓縮所有檔案類型（預壓縮的檔案類型除外） </b> — 輸入 [!DNL Apache] Web伺服器、 [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html) 可以使用，但請確定 <b>`Vary: User-Agent` </b>標題<b> </b>未設定。  在Microsoft IIS中，使用 [動態壓縮](https://docs.microsoft.com/en-us/iis/configuration/system.webserver/httpcompression/).

   [!DNL Apache] 配置示例（僅指定某些內容類型以避免預壓縮的檔案類型）:

   `AddOutputFilterByType DEFLATE text/html text/plain text/xml text/css text/javascript application/javascript`
7. <b>啟用 [/serveStaleOnError](https://helpx.adobe.com/experience-manager/kb/ServeStaleContentOnError.html)</b> 在 `/cache` 設定 — 當AEM例項執行錯誤時，提供舊快取檔案。
8. <b>新增 [/gracePeriod](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#configuring-the-dispatcher-cache-cache)</b> 到 `/cache` 設定 — 定義在最後一個內容發佈事件（「啟用」）後，快取仍可提供過時、自動失效的資源的秒數。  這可減少大型內容發佈活動（例如「樹狀檢視啟動」）期間，傳回至發佈例項的請求數。
...
10. <b>快取控制項和上次修改的響應標頭</b>  — 使用<b> [/headers](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#CachingHTTPResponseHeaders)</b> 快取HTTP回應標題的設定 <b>`Cache-Control`</b> 和 <b>`Last-Modified` </b>(和/或 <b>`ETag`</b> 標題(如果您是從AEM傳送)。  這有助於簡化和最佳化CDN和瀏覽器層級的快取。  快取這些標題可讓僅AEM設定標題，而非Web伺服器本身。  請注意，執行此動作時，您需要開始從AEM應用程式傳送標題。
11. <b>盡可能長時間快取內容</b> 和 <b>減少傳回AEM的請求</b>  — 通過啟用 [重新擷取](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html#refetching-flush) 刷新所有刷新代理。  或使用 [<b>/enableTTL</b>](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html#use-ttls) 設定 <b>`Cache-Control: max-age=...`</b> 盡可能將標頭快取檔案。  請參閱 [low](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html#use-ttls) 以取得本主題的詳細資訊。

<br><br><br><br><br><br>使用TTL<br><br><br><br><br><br>
截止日期 [!DNL Dispatcher] 版本4.1.11, [/enableTTL 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#ConfiguringTimeBasedCacheInvalidationenableTTL) 可在 `.any` 檔案設定。  此設定會讓Dispatcher遵循HTTP中設定的快取有效期 `Cache-Control` 回應標題。  換言之，Dispatcher的運作方式與CDN類似，當檔案過期時，會發生主要形式的快取失效。  實作此項目並開始傳送後 <b>`Cache-Control: max-age=...` </b>針對AEM的所有回應，則您可以在發佈執行個體中安全地停用dispatcher排清代理。
...
1. 修改AEM應用程式中要傳送的原始碼 <b>`Cache-Control` </b>標題和 <b>`Last-Modified` </b>適用於尚未設定的所有請求。
2. 安裝 [!DNL Dispatcher] 4.1.11或更新版本。
3. 設定 <b>[/enableTTL 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#ConfiguringTimeBasedCacheInvalidationenableTTL)</b> 在 `.any` 網站的伺服器陣列設定。
4. 設定 <b>[/headers](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#CachingHTTPResponseHeaders) </b>快取配置 <b>`Cache-Control`</b> 和 <b>`Last-Modified`</b> 標題。
5. 重新啟動網頁伺服器。


<b>二。 在發佈執行個體上停用Dispatcher排清代理：</b>

Dispatcher現在會使用 `Cache-Control` 控制快取檔案失效的標題。  因為如此，就不再需要從發佈例項排清調度程式。

1. 前往 `/etc/replication/agents.publish.html` 在每個發佈例項上。
2. 轉到每個刷新代理的配置並禁用代理。


<b>三。 允許來自製作例項的手動Dispatcher排清請求：</b>

現在，由於已禁用刷新代理，因此您完全依賴 <b>`Cache-Control` </b>頁首，以控制dispatcher上何時重新整理內容。  您仍可讓使用者手動清除Dispatcher快取：
...
由於在步驟2中刪除了檔案，下次使用者請求此類檔案時， `/content/foo.html` 或 `/content/foo.json`，當檔案「重新擷取」時，對相同檔案的後續要求也會傳送至發佈執行個體，直到快取檔案為止。  若是回應緩慢或流量大的頁面（例如首頁），這可能會導致發佈執行個體層級泛濫。
