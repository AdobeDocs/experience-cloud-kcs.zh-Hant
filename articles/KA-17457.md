...
2. 如果AEM java進程導致CPU利用率高，請在作業系統級別檢查：    
...
4.檢查系統維護過程。 看這個 [文章](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/monitoring-and-maintaining.html)如需AEM維護的詳細資訊，請確定您對AEM執行正確維護，包括： — 修訂清除（僅限MongoMK和Database DocumentNodeStore） — 每日或更頻繁 — 離線Tar壓縮（僅限TarMK） — 每週 — 資料儲存垃圾收集（僅限FileDataStore或S3 DataStore的系統） — 每週 — 工作流清除 — 每週 — 每週 — 版本清除 — 每週 — 每週 — 審核AEM快取層級實施的策略。
6.檢閱網站的快取。
...
1. 在AEM例項上開啟這些URL以檢視關於非同步索引器http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dasync%2Ctype%3DIndexStats http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dfulltext-async%2Ctype%3DIndexStats  - This僅適用於AEM6.2和更新版本的URL統計資料
2. 在這些頁面上，核取下列欄位：
