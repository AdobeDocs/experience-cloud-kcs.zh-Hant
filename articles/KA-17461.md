...
2. <b>&#x200B; s-maxage </b>- <b>s-maxage </b>指令 <b>快取控制</b> 標頭可讓您為共用快取（例如CDN）設定不同的TTL。  設定此值時，瀏覽器會使用中設定的值 <b>最大年齡</b> 其他快取會尊重 <b>s-maxage</b> 設定。
   ...
   <b>  </b><b>快取控制：max-age=600, s-maxage=300</b>
...
在Google Chrome中測試快取時，如果您是透過https測試，且您有自行簽署的憑證，則系統不會快取任何內容。  當有不受信任或無效的憑證時，Chrome不會快取回應或執行重新驗證。
...
- 在Dispatcher層級上啟用GZip壓縮：Apache — 使用 [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html).  請留意mod_deflate使用Vary。  在某些情況下，Vary標題會導致CDN和瀏覽器完全略過快取。
...
- 不允許對已壓縮的大檔案或檔案進行gzip壓縮。  請注意，大部分的影像和視訊格式已經預先壓縮。  在Web伺服器級別即時壓縮它們會帶來極高的效能成本。
   ...
   - 在IIS上，這可以透過 [dynamicTypes設定](https://docs.microsoft.com/zh-tw/iis/configuration/system.webserver/httpcompression/).
- 如果您的CDN提供者支援 [邊緣側包括](https://en.wikipedia.org/wiki/Edge_Side_Includes) (ESI)，然後利用此功能。 AEM元件可使用ESI進行劃分。 若要這麼做，請使用Apache Sling Dynamic Includes(https://github.com/Cognifide/Sling-Dynamic-Include)或實作自訂解決方案。 如果您有相當靜態的頁面，但在頁面的幾個部分提供更多動態內容，則此功能會很實用。 在這些情況下，您實際上會將頁面分割成多個CDN檔案。 這樣，您就可以在不同時段內快取頁面的不同部分。
...
由於這是較大的主題，請參閱 [這篇文章](https://helpx.adobe.com/tw/experience-manager/kb/optimizing-the-dispatcher-cache.html) 以取得如何最佳化dispatcher快取的詳細資訊。
...
2. 如果網站有個人化/動態內容：
   ...
   1. 使用 [Apache Sling Dynamic Includes](https://helpx.adobe.com/tw/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) 來劃分內容，以便在不同時段內快取頁面的不同部分。
