...
| `Include /etc/httpd/conf.d/variables/ams_default.vars````IfModule` &#39;disp_apache2.c``    ``DispatcherConfig conf.dispatcher.d/dispatcher.any``    ``DispatcherLog logs/dispatcher.log``    ``DispatcherLogLevel ${DISP_LOG_LEVEL}``    ``DispatcherDescineRoot 0``    ``DispatcherUseProcessedURL 1``/``IfModule&quot;&#39; |
| --- |

<br><br><br><br><br> <br><br>
了解 <b>DispatcherLogLevel</b> 指令的變數為DISP_LOG_LEVEL，而不是您在那裡看到的正常值。  在程式碼的該節上方，您也會看到變數檔案的include陳述式。  變數檔案 */etc/httpd/conf.d/variables/ams_default.vars*是我們下一個想看的地方。  以下是變數檔案的內容：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
如果您看到DISP_LOG_LEVEL變數的當前值是「資訊」。  我們可以調整以追蹤、除錯，或您選擇的數字值/層級。  現在，每個控制日誌級別的位置都會自動調整。
...
<br><br><br><br><br> <br><br>
現在，Apache webserver知道網域是某些內容應會產生的流量，但我們仍需通知Dispatcher模組，它有新的網域名稱需遵守。  首先，我們將建立一個新的\*_vhost.any檔案 <b>/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any</b> 在檔案里，我們要放入我們要遵守的域名：
<br><br><br><br><br> <br><br><br><br>
