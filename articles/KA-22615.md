...
客戶正在開發網站，應使用AEM Headless解決方案從其AEM執行個體擷取資料。

若要設定端點，請參閱中的教學課程 `[` 1`]`  已遵循。

發佈所有內容片段後，所有模型、內容、參考、資產和查詢，如中所述 `[` 2`]` ，嘗試從發佈執行個體擷取任何內容片段時，會收到404錯誤，導致無法從發佈者讀取內容。
...
`[` 1`]`  aem_base_version： 6.5 aem_envId： xxxxxxx aem_envType： dev aem_program_id： xxxxxx aem_release_id： xxxxx aem_service： cm-pxxxxxx-exxxxxxx aem_tenant： ns-team-aem-cm-prd-nxxxxxx aem_tier：發佈後端： publishfarm/- cache： <b>已封鎖</b>
叢集： `<` 叢集`>`
代碼： - file_path： /var/log/aem/dispatcher.log host： publish-pxxxxxx-exxxxxxx.adobeaemcloud.com方法：POST名稱空間： ns-team-aem-cm-prd-nxxxxxx orig_time： 25/Jul/2023:15:39:11 +0000路徑： /content/cq：graphql/graphql/endpoint.json pod_name： `<` pod_name`>`
pod_uid： `<` pod_uid`>`
