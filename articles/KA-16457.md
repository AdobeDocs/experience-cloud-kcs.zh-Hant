...
您觀察 *SegmentNotFoundException* AEM和AEM無法如預期運作。
...
*警告：<b>*</b>此流程會將系統中的資料復原到之前的時間點。如果您想避免遺失系統中的變更，可以嘗試 <b>選項B</b> 而改為位於下方。

若要執行檢查和還原，請遵循下列步驟：

1. 下載 `oak-run` 從這裡的jar檔案 [https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/](https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/)
2. 停止 AEM。
3. 執行此命令：
...
4. 透過編輯將存放庫恢復到此版本：
   ...
   如果您想瞭解將存放庫還原到的日期和時間，請在 `segmentstore` 資料夾(取代 *afdb922d-ba53-4a1b-aa1b-1cb044b535cf* 將最新的良好修訂版加入您的 `journal.log`)：
...
*注意：*以下程序適用於進階使用者。刪除損壞的節點時，您需要確保它們不是系統節點(例如 `/home`， `/jcr:system`、等) 。
或者，如果它們是系統節點，那麼您需要確保可以還原它們。
如果您不確定，請向AEM客戶服務團隊尋求此處記錄的步驟的協助。

1. 停止 AEM。
2. 使用Oak run主控台並載入 `childCount` 用來識別區段存放區中損壞節點的groovy指令碼：
   ...
   在某些情況下，該問題與二進位屬性和 `childCount` groovy指令碼找不到任何損壞的節點。

   在這些情況下，您可以使用以下命令代替，它將讀取周遊期間遇到的每個二進位檔案的前1024個位元組（請注意，此命令會較慢，並且只應在上述命令未傳回預期結果時使用）：

   `countNodes(session.workingNode,true)`


3. 使用移除最後一個命令輸出中列出的所有已識別損壞節點 `rmNodes.groovy`.
   ...
   其中損壞的節點路徑為步驟2中取得的路徑，例如： `/content/dam/test.txt/jcr:content/renditions/original/jcr:content/`.注意：使用時 `oak-run.jar` 1.6.13版及更高版本，設定 `--read-write` JVM引數（如果您遇到錯誤），例如：
...
4. 對步驟 2 中找到的所有節點重複步驟 3。
   ...
   重新執行，確定刪除這些找到的三個損壞的路徑 `rmNode` 這些路徑上的指令。
   ...
   Oak-run jar的修補版本有什麼作用？

   此版本的jar在壓縮時會略過無法讀取的二進位檔案，將它們取代為0位元組的二進位檔案並記錄例外狀況和syserr的路徑。
...
6. 執行離線壓縮。
   ...
   如果您不知道如何執行離線壓縮，請參閱 [Oak離線壓縮指示](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b) 在GitHub摘要上。
