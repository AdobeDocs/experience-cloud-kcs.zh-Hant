...
本檔案將說明Dispatcher快取的發生方式，以及如何設定<br><br> <br><br>[目錄](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html)
...
我們刻意將已發佈的工作量與製作工作量分開，因為當Apache在DocumentRoot中尋找檔案時，它不知道它來自哪個AEM執行個體。 因此，即使您已在製作伺服器陣列中停用快取，如果作者的DocumentRoot與發佈者相同，則會在有檔案時從快取中提供檔案。 這表示您會從已發佈的快取中提供作者檔案，並為訪客提供非常糟糕的混合比對體驗。 為不同的已發佈內容保留單獨的DocumentRoot目錄也是一個非常糟糕的主意。 您必須建立多個重新快取的項目，這些項目在客戶端等站點之間沒有差別，並且必須為您設定的每個DocumentRoot設定一個複製刷新代理。 每次頁面啟動時，增加頭頂的排清量。 請仰賴檔案的命名空間及其完整快取路徑，並避免發佈網站出現多個DocumentRoot。



<b>組態檔</b>

Dispatcher控制任何伺服器陣列檔案的/cache {區段中可快取的資格。 
...
我們有快取/lib的規則，因為它們是基準AEM安裝的一部分，且會變更，直到您安裝Service Pack、Cumulative Fix Pack、升級或Hotfix為止。 因此，快取這些元素很有意義，而且使用網站的使用者的製作體驗確實有巨大的好處。
...
這是Dispatcher功能的其中一項。 如果發佈者正在載入中，或已停止回應，通常會擲回502或503 http回應代碼。 如果發生此情況並啟用此功能，即使內容不是新復本，系統仍會指示Dispatcher將任何內容仍保留在快取中的內容，以盡量提供。 如果您有，最好提供某個功能，而不只是顯示沒有功能的錯誤訊息。
...
Dispatcher模組的一般行為是，如果請求在URI中有查詢參數(通常如/content/page.html所示)<b>?myquery=value</b>)會略過快取檔案，然後直接前往AEM例項。 它認為此請求是動態頁面，不應進行快取。 這可能會導致我對快取效率造成影響。

如果AEM中的頁面在地址行中採用GET引數/查詢參數，可協助頁面運作，但未呈現不同的html則是此設定元素的理想候選項。您可以告訴Dispatcher要忽略哪些引數，並仍會快取頁面。
例如，某人建立了社交媒體深層連結參考機制，此機制使用URI中的引數參考來了解人員來自何處。
...
現在，當Dispatcher看到請求時，會忽略請求的查詢參數為？reference，仍會快取頁面的事實



<b>快取回應標題</b>

很明顯，Dispatcher會快取.html頁面和clientlibs，但您是否知道它也可以將特定回應標題與相同名稱但副檔名為.h的檔案中的內容一起快取。 這不僅可讓內容的下一個回應，也可讓快取中隨附的回應標題。

AEM可處理不只是UTF-8編碼

有時，項目會有特殊標頭，可協助控制快取TTL的編碼詳細資訊，以及上次修改的時間戳記。

預設會移除快取時的這些值，而apache httpd webserver將自行執行處理資產的工作，使用其一般的檔案處理方法，這類方法通常僅限根據副檔名mime類型猜測。

如果您有Dispatcher快取資產和所需的標題，便可公開適當的體驗，並向用戶端瀏覽器保證所有詳細資訊。
...
如果您有5個使/content/exampleco/en/無效的請求，則所有請求都會在3秒內發生。

使用此功能，將快取目錄/content/exampleco/en/無效5次

開啟此功能並將其設為5秒，會使快取目錄/content/exampleco/en/失效<b>once</b>
...
請記住，AEM仍需設定為傳送TTL標頭，Dispatcher才能執行。 切換此功能只會讓Dispatcher知道何時要移除AEM已傳送快取控制標題的檔案。 如果AEM未開始傳送TTL標頭，則Dispatcher不會在此執行任何特殊動作。
