...
description: 瞭解如何解決您無法在作者中載入DAM資產主控台的AEM問題。 清除工作流程例項。
solution: Experience Manager
product: Experience Manager
applies-to: "Experience Manager"
keywords: 「KCS、無法載入DAM資產控制檯、作者、AEM、工作流程」
...
如果您在存取Author中的資產AEM主控台時看到空白畫面，可能是因為太多工作流程例項導致錯誤 `[` 1`]` . 您可以清除這些例項來解決此問題。
...
嘗試使用「 」存取資產AEM主控台時`/assets.html/content/dam`「畫面為空白。
...
Assets和Sites UI必須查詢其所列出專案的工作流程狀態。 因此，如果工作流程例項過多，頁面將無法載入並發生錯誤 `[` 1`]`  會出現在記錄中。
