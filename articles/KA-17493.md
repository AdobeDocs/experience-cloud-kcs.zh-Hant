...
![作者啟用內容，這會觸發發佈者傳送排清請求給dispatcher](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/dispatcher-flushing-order-of-events.png "dispatcher-flushing-order-of-events")
...
當發佈者收到檔案時，會將復寫代理程式設定為指向在接收事件時觸發的傳送器。  然後它會序列化排清請求，並將其發佈到排程程式。
<br>作者復寫代理程式<br>
以下是已設定標準復寫代理程式的一些熒幕擷取畫面範例
![AEM網頁/etc/replication.html中的標準復寫代理程式熒幕擷圖](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/author-rep-agent-example.png "author-rep-agent-example")
...
![AEM網頁/etc/replication.html中的標準排清復寫代理程式熒幕擷圖](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")<br>DISPATCHER FLUSH復寫接收虛擬主機<br>
Dispatcher模組會尋找特定標頭，以瞭解POST請求何時可以傳遞給AEM轉譯器，或者該請求是否已序列化為排清請求，且需要由Dispatcher處理常式本身處理。  以下是顯示這些值的設定頁面熒幕擷圖：
![「序列化型別」顯示為「Dispatcher排清」的主要設定畫面設定索引標籤的圖片](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent1.png "disp-flush-agent1")
預設設定頁面會顯示 <b>序列化型別</b> 作為 *Dispatcher排清* 並設定錯誤等級
![復寫代理程式之傳輸頁簽的熒幕擷圖。  這會顯示將排清請求發佈到的URI。  /dispatcher/invalidate.cache](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent2.png "disp-flush-agent2")
在傳輸標籤上，您可以看到URI設定為指向將接收排清請求的傳送器的IP位址。  路徑/dispatcher/invalidate.cache並非模組判斷其是否為排清的方式，而只是您可於存取記錄中看到的明顯端點，用以得知其為排清請求。  在「擴充功能」標籤上，我們將瀏覽現有的專案，以判斷這是傳送器模組的排清請求。
![復寫代理程式「延伸」索引標籤的熒幕擷圖。  請注意與傳送的POST請求一起傳送以通知排程程式排清的標頭](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent3.png "disp-flush-agent3")
...
- Host
   - 這是欺騙Host Header以鎖定在Dispatcher Apache Web Server (/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost)上設定之特定VirtualHost的位置。  這是硬式編碼值，符合aem_flush.vhost檔案的 *伺服器名稱* 或 *ServerAlias*

![標準復寫代理的畫面，顯示當從作者發佈內容收到來自復寫事件的新專案時，復寫代理會作出反應並觸發](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent4.png "disp-flush-agent4")
在「觸發器」索引標籤上，我們會記下我們使用的切換觸發器及其功能
...
此排清範例會更新該目錄中的.stat檔案來排清/content/dam路徑。
<br>.stat檔案<br>
排清機制本質上很簡單，我們要說明 <b>.stat</b> 在檔案根目錄中產生的檔案，其中會建立快取檔案。

在.vhost和_farm.any檔案中，我們會設定檔案根指令，以指定當一般使用者的請求傳入時，快取的位置以及儲存/服務檔案的位置。

如果您要在Dispatcher伺服器上執行以下命令，您會開始找到.stat檔案
...
下圖是當快取中有專案，且Dispatcher模組已傳送並處理排清請求時，此檔案結構的外觀
![含有顯示統計層級之內容和日期的statfiles](https://helpx.adobe.com/content/dam/help/en/experience-manager/kb/ams-dispatcher-manual/disp-flushing/jcr_content/main-pars/image_2035486865/dispatcher-statfiles.png "dispatcher-statfiles")<br>stat檔案層級<br>
請注意，每個目錄中都會有.stat檔案。  這是表示已發生排清的指標。  在上述範例中 <b>statfilelevel</b> 設定已設為 <b>3</b> 在對應陣列設定檔案中。

statfilelevel設定表示模組將周遊並更新.stat檔案的資料夾深度。  .stat檔案是空的，只不過是帶有datestamp的檔案名稱，甚至可以手動建立，但可在Dispatcher伺服器的命令列上執行touch命令。
...
將statfilelevel設定在合理的層級。  檢視您的資料夾結構，並確定其設定為允許簡潔的排清，而不需周遊太多目錄。   在系統效能測試期間進行測試並確定它符合您的需求。
...
您可以指定讓Dispatcher填入並管理的目錄作為快取目錄。

注意：

此目錄應該與網頁伺服器設定要使用的網域之Apache檔案根目錄設定相符。
...
清除/content/dam/brand1/en/us/logo.jpg時，若將/statfileslevel設定在下列數字（檔案根目錄為/var/www/html/）將產生下列結果
...
請記住，發生時間戳記交握時，會尋找最近的.stat檔案。

.stat檔案層級0且stat檔案僅位於/var/www/html/.stat中，表示/var/www/html/content/dam/brand1/en/us/下的內容會尋找最近的.stat檔案，並遍歷5個資料夾以尋找層級0唯一的.stat檔案，並比較其日期。  這表示在如此高的層級排清實際上會使所有快取的專案失效。
...
| `$ stat``/mnt/var/www/html/content/customer/en-us/``.stat``  ``File: `.stat&#39;``  ``大小： 0個區塊： 0 IO區塊： 4096一般空白``file``裝置： ca90h``/51856d`    `Inode: 17154125    Links: 1``存取： (0644``/-rw-r--r--``) Uid： ( 48/ apache) Gid： ( 48/ apache)``Access: 2019-11-13 16:22:31.000000000 -0400``修改： 2019-11-13 16:22:31.000000000 -0400``Change: 2019-11-13 16:22:31.000000000 -0400`<br> <br>`$ stat``/mnt/var/www/html/content/customer/en-us/logo``.jpg``檔案： `logo.jpg'``  ``Size: 15856           Blocks: 32          IO Block: 4096   regular``file``Device: ca90h``/51856d`    `Inode: 9175290    Links: 1``Access: (0644``/-rw-r--r--``)  Uid: (   48/  apache)   Gid: (   48/  apache)``Access: 2019-11-11 22:41:59.642450601 +0000``Modify: 2019-11-11 22:41:59.642450601 +0000``Change: 2019-11-11 22:41:59.642450601 +0000` |
| --- |
