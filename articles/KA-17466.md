...
此外，ACL不會套用至結果，所以對目前工作階段不可見的節點仍會包含在傳回的計數中。 因此，傳回的計數可能會高於實際結果數，而準確的計數只能透過逐一檢視結果來決定：
   ...
   - <b>Lucene索引設定</b>\
     ...
     另請參閱 [https://jackrabbit.apache.org/oak/docs/query/lucene.html](https://jackrabbit.apache.org/oak/docs/query/lucene.html) 如需可用引數的詳細資訊由於有些路徑不需要索引，您可以執行下列動作：在CRXDE Lite中，前往/oak：index/lucene，使用下列值設定名為excludedPaths的多值字串屬性（字串） /var、/etc/workflow/instances、/etc/replication。
   - <b>資料存放區</b>\
