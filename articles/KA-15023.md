...
description: 「瞭解如何在Adobe Launch中實施作業視訊分析。」
solution: Analytics
product: Analytics
applies-to: "Analytics"
keywords: 「KCS、AdobeLaunch、影片、Analytics、Launch、Adobe」
...
按一下「事件和條件」下的+按鈕，建立條件以啟動YouTube影片。 例如，您已新增「元素存在」條件來尋找特定div「播放器」，以啟動YouTube影片。
...
1. 從「擴充功能」下拉式清單中選取「核心」。
2. 從「動作型別」下拉式清單中選取「自訂程式碼」。
3. t-size：12.0pt」`>` 在名稱文字方塊中新增動作名稱。
4. 選取「語言」下的「Javascript」。
5. 勾選「全域執行」。
6. 按一下「開啟編輯器」。
...
2. 從「動作型別」下拉式清單中選取「自訂程式碼」。
3. 在名稱文字方塊中新增動作名稱。
4. font-size：12.0pt」`>` 選取「語言」下的「Javascript」。
5. pan style=&quot;font-size：12.0pt&quot;`>` 勾選「全域執行」。
6. style=&quot;font-size：12.0pt&quot;`>` 按一下「開啟編輯器」。
7. span style=&quot;font-size：12.0pt&quot;`>` 新增訪客ID服務程式碼。
