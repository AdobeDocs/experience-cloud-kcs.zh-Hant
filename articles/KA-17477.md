...
| FILENAME.vhost | 分階段： /etc/httpd/conf.d/available_vhosts/<br><br>活動：<br><br>/etc/httpd/conf.d<br><br><u>*注意：.vhost檔案不會複製到enabled_vhosts資料夾中，但使用symlink到available_vhosts/ .vhost檔案的相對路徑</u><br><br> | \*.vhost（虛擬主機）檔案是VirtualHosts項，用於匹配主機名，並允許Apache使用不同規則處理每個域通信。 從.vhost檔案中，將包含其他檔案，例如重寫、白名單等。 |
...
<br><br><br><br> <br><br>Apache基線包含<br><br><br><br><br><br>![apache基線包含順序。  apache二進位檔以httpd.conf開頭，會對「conf.d/」加上includeoptional *.conf和conf.modules.d/*.conf目錄](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-Baseline-Includes.png "Apache-Webserver-Baseline-Includes")<br><br><br><br> <br><br>
...
| 1<br>  2<br>  3<br>  4<br>  5<br>  6<br>  7<br>  8<br>  9<br>  10<br>  11<br>  12<br>  13<br>  14<br>  15<br>  16<br>  17<br>  18 | &#39;包含``/etc/httpd/conf``.d``/variables/weretail``.var``VirtualHost *:80``    ``ServerName   ``&quot;${MAIN_DOMAIN}&quot;``    ``目錄/``        ``包括``/etc/httpd/conf``.d``/whitelists/weretail``*_whitelist.rules``        ``IfModule disp_apache2.c``            ``....SNIP....``            ``SetHandler Dispatcher-handler``        ````/IfModule````        ``....SNIP....``    ````/Directory````    ``....SNIP....``    ``IfModule mod_rewrite.c``        ``ReWriteEngine開啟``        ``LogLevel警告重寫：trace1``        ``包括``/etc/httpd/conf``.d``/rewrites/weretail_rewrite``.rules&quot; &quot;&quot;/IfModule&quot;&quot;&quot;/VirtualHost&quot;&#39; |
| --- | --- |
...
接下來，讓我們看 <b>/etc/httpd/conf.dispatcher.d/filters/weretail_search_filters.any</b>:
<br><br><br><br><br> <br><br><br><br>
