...
description: 瞭解如何修正發生錯誤 — Adobe Experience Manager中缺少end_aem_createsegmentstore屬性的問題。
solution: Experience Manager
product: Experience Manager
applies-to: Experience Manager
keywords: KCS、管道、AEM、CI/CD、錯誤
...
「當管道中發生錯誤時： `The end_aem_createsegmentstore property is missing`，請依照本文提供的說明修正問題。」
...
此 <b>持續整合</b> 和 <b>持續部署</b> <b>(CI/CD)</b> 在此期間管道持續失敗 *部署至開發* 階段。
<br>由於程式碼基底，部署記錄未顯示任何錯誤，但出現以下錯誤：<br> <br>`[` 1`]`  
...
檢查管道，發現它因錯誤而失敗 *errorCode*： *BLOCKED_QUEUE*.

這表示復寫佇列遭到封鎖，阻礙其他可變內容的安裝。

在記錄中，錯誤 <b>`[` 1`]` </b> 在部署時間範圍內存在。 很可能是節點 —  <b>`[` 2`]` </b> 發行者及/或預覽伺服器上不存在，且復寫失敗。
