...
一般而言，在「報表套裝管理器 — 應用程式管理 — 應用程式分類」下存取「行動裝置分類」時，可能會擲回下列提及的訊息 — 

行動裝置分類：&#39;*此報表套裝尚未啟用此功能。 請連絡您的客戶支援代表以啟用。*&#39;（請參閱下文）。

![](assets/___461a6949-17c6-ec11-a7b6-0022480a10ee___.png)

若要為行動應用程式維度啟用分類，使用者必須先為該報表套裝啟用「應用程式報表」。


## 解析度

<b>先決條件</b>:產品管理員權限。<br><br> <br><br><b>以下是啟用應用程式報告的步驟</b>: <br><br>
- 前往 *Analytics管理員報表套裝選取所需的報表套裝編輯設定應用程式管理應用程式報表* 在「報表套裝管理器」中。
- 按一下「<b>*啟用最新應用程式報表</b>*.&#39; 您也可以根據您的需求來啟用其他功能。
