...
4. 在影像中 *標籤* 將任何影像設為縮圖
5. 轉出頁面
6. 現在，請前往 <b>[!DNL Sites] UI</b>
7. 開啟的頁面屬性 `/content/we-retail/us/en/women` an
8. 在 *[!UICONTROL 縮圖]* 按一下 *[!UICONTROL 產生預覽]* 並儲存變更
9. 現在重新轉出中的頁面 `/siteadmin`
...
轉出期間發生此問題，因為AEM無法建立 `dam:thumbnails` 下節點 `jcr:content` 即時副本頁面(`/content/we-retail/us/en/women/jcr:content/image/file/jcr:content`)和原因是在轉出mixinType期間 `dam:Thumbnails` 未套用至即時副本頁面節點 `/content/we-retail/us/en/women/jcr:content/image/file/jcr:content` 和 `dam:Thumbnails` 只能在下方建立資料夾 `dam:Thumbnails` nodeType。

MSM預設行為是在轉出0期間忽略新增至來源節點的mixin，因此它不會新增至 `/content/we-retail/us/en/women/jcr:content/image/file/jcr:content`.
