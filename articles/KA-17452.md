...
<b>如何在Linux、UNIX或Windows上從JVM取得執行緒轉儲？</b>
...
- 在Linux和UNIX中，您需要以擁有java進程的用戶身份運行該命令：
   ...
   (`<` java-user`>`  應取代為Java程式執行方式的使用者id)
...
- 如果jstack命令(<b>jstack -l `<` pid`>`  `>` `>`  threaddumps.log</b>)擲回錯誤 `[` 1`]`  下面，以擁有java進程的用戶身份運行該命令。  例如:
...
就這樣運行：
<b>sudo -u *`<` 使用者`>`*jstackSeries.sh `<` *pid`>`* `<` *aemserveruser`>`* `<` *計數`>`* `<` *延遲`>`</b>*
...
`[` 0`]`  [PsExec v2.42](https://learn.microsoft.com/en-us/sysinternals/downloads/psexec) 在Microsoft檔案中。
`[` 1`]`  [TDA — 線程轉儲分析器](https://github.com/irockel/tda) 在Github.com上的irockel/tda。
`[` 2`]`  [Java線程轉儲分析器](https://fastthread.io/) 在快速線程上。
`[` 3`]`  [IBM線程和監視器Dump Analyzer for Java](https://www.ibm.com/support/knowledgecenter/en/SSLLVC_5.0.0/com.ibm.esupport.tool.tmda.doc/docs/readme.htm) IBM支援助理檔案。
