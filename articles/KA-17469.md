...
<br><br><br>問題：&quot;獲取blobId的InputStream時出錯&quot;<br><br><br><br><br><br>
您有 [FileDataStore](https://docs.adobe.com/docs/en/aem/6-0/deploy/upgrade/data-store-config.html#Data%20Store%20Configurations) 在Adobe Experience Manager 6.x / Oak 1.x系統中設定，並在 `error.log` 您會看到「取得blobId的InputStream時發生錯誤」。
...
要恢復缺少的檔案，請執行以下步驟。
<br><br><br><br> <br><br>1. 安裝最新CFP/SP<br><br><br><br> <br><br>
您可以在 [Adobe套件共用](https://www.adobeaemcloud.com/content/packageshare.html).
<br><br><br><br> <br><br>2. 運行資料儲存一致性檢查<br><br><br><br> <br><br>
使用 `oak-run` jar 1.6.6及更高版本，請運行以下命令：
...
<br><br><br><br> <br><br>3. 建立缺少檔案的路徑清單<br><br><br><br> <br><br>
恢復缺少的檔案的下一步是編譯缺少的檔案路徑的完整清單。

1. 搜索一致性檢查輸出，以查找所有出現的「未找到記錄」錯誤。

a. <b>在 [!DNL Linux] 或 [!DNL Unix]:</b> 使用此命令將缺少的檔案清單輸出到檔案 *missing_ds_files.txt*:
<br><br><br><br><br> <br><br><br><br>
...
\*\*如果Adobe Experience Manager例項因「blobId」錯誤而未啟動，請在下方搜尋您的記錄檔 `crx-quickstart/logs` 針對「取得blobId的InputStream時發生錯誤」錯誤的所有發生次數：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
如果命令正常工作，則 `missing_ds_files.txt` 會如下所示：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
b. <b>在 [!DNL Windows]:</b>  使用文字編輯器，例如 [!DNL Textpad] 或 [!DNL Notepad++] 查找「未找到記錄」的所有出現次數。  然後，在發現所有這類事件後，使用宏、寫指令碼或手動複製檔案名並將其構建到新文本檔案中來提取檔案名。
...
<br><br><br><br><br> <br><br>
記錄id為 `add1dd8fc5093b27b1fae1b753cb48b24ef3231f` 而檔案路徑是 `ad*/d1/dd/add1dd8fc5093b27b1fae1b753cb48b24ef3231f*`
<br><br><br><br> <br><br>4. 恢復缺少的檔案<br><br><br><br> <br><br>
...
在 [!DNL Linux]，您可以登入每個正在運作的Adobe Experience Manager執行個體，並使用rsync等命令來複製其中任何遺失的檔案。  例如，從缺少檔案的伺服器上的資料存放區目錄執行：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
命令會執行rsync，該rsync會複製列於 `missing_ds_files.txt` 伺服器中存在。
<br><br><br><br> <br><br>5。 清除不可恢復的檔案引用<br><br><br><br> <br><br>
如果您無法從備份或其他Adobe Experience Manager執行個體復原部分檔案，請清理或修正錯誤的資料存放區參考。  在步驟4中執行DataStore一致性檢查時，請重新執行。  您會收到遺失檔案的最新清單。

查看列出的每個節點路徑，這些路徑引用缺少的資料儲存檔案。  檢閱使用者上傳至頁面的任何遺失DAM資產或檔案。  讓他們重新上傳任何需要的遺漏項目。  您可以透過Adobe Experience Manager使用者介面安全地刪除不需要的任何項目。  如果下方缺少任何項目 `/var/audit` 或 `/var/eventing` 可以安全地刪除。  對於您不確定的任何檔案，請轉到 [此處](https://helpx.adobe.com/tw/marketing-cloud/contact-support.html) 並聯絡AEM支援團隊以取得協助。
<br><br><br><br> <br><br>6. 重新索引所有非同步Oak索引<br><br><br><br> <br><br>
由於索引檔案儲存在Oak存放庫中，因此也可能遺漏了 `/oak:index` 基於lucene的索引。

系統不會在 `/system/console/repositorycheck` 在儲存庫周遊期間隱藏一致性檢查。  很可惜，這些檔案在不同的AEM例項間不會相符。
...
1. 下載 `oak-run` 1.0.x版符合您在Oak環境中安裝的版本： [https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/](https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/)

2. 上傳 `oak-run` 到Adobe Experience Manager伺服器。
...
<br><br><br><br><br> <br><br>
5.重新啟動Adobe Experience Manager，並監視您的記錄檔，以取得來自 `org.apache.jackrabbit.oak.plugins.index`.  如果要查看有關索引的詳細資訊，請轉至 `/system/console/slinglog` UI和啟用的偵錯層級記錄 `org.apache.jackrabbit.oak.plugins.index`.
